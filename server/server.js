...
const databasePath = process.env.VERCEL ? join("/tmp", "kabadiwala.db") : join(__dirname, "kabadiwala.db");
...
if (database.prepare("SELECT COUNT(*) AS count FROM prices").get().count === 0) {
  const insertPrice = database.prepare("INSERT INTO prices (id, name, hiName, price, unit, trend) VALUES (?, ?, ?, ?, ?, ?)");
  const insertRecycler = database.prepare("INSERT INTO recyclers (id, name, dist, priceMul, verified, pickup) VALUES (?, ?, ?, ?, ?, ?)");
  const insertLot = database.prepare("INSERT INTO lots (lotId, materialId, material, weight, estimatedVal, collectorName, status, createdAt, synced) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");

  database.exec("BEGIN IMMEDIATE;");
  try {
    seed.prices.forEach((item) => insertPrice.run(item.id, item.name, item.hiName, item.price, item.unit, item.trend));
    seed.recyclers.forEach((item) => insertRecycler.run(item.id, item.name, item.dist, item.priceMul, item.verified ? 1 : 0, item.pickup ? 1 : 0));
    seed.lots.forEach((item) => insertLot.run(item.lotId, item.materialId, item.material, item.weight, item.estimatedVal, item.collectorName, item.status, item.createdAt, item.synced ? 1 : 0));
...
function getAnalytics() {
  ...
  const processedWeight = completedLots.reduce((total, lot) => total + lot.weight, 0);
  const recoveredValue = completedLots.reduce((total, lot) => total + lot.estimatedVal, 0);
...
export async function handleRequest(request, response) {
  ...
  if (pathname === "/health" && request.method === "GET") return sendJson(response, 200, { ok: true });

  if (pathname === "/prices" && request.method === "GET") return sendJson(response, 200, database.prepare("SELECT * FROM prices ORDER BY id").all());
  const materialIdMatch = pathname.match(/^\/prices\/([^/]+)$/);
  if (materialIdMatch && request.method === "GET") {
    const material = database.prepare("SELECT * FROM prices WHERE id = ?").get(decodeURIComponent(materialIdMatch[1]));
  ...
  if (pathname === "/recyclers" && request.method === "GET") {
    return sendJson(response, 200, database.prepare("SELECT id, name, dist, priceMul, verified, pickup FROM recyclers ORDER BY id").all().map((item) => ({ ...item, verified: Boolean(item.verified), pickup: Boolean(item.pickup) })));
  }
  const recyclerIdMatch = pathname.match(/^\/recyclers\/([^/]+)$/);
  if (recyclerIdMatch && request.method === "GET") {
    const recycler = database.prepare("SELECT id, name, dist, priceMul, verified, pickup FROM recyclers WHERE id = ?").get(decodeURIComponent(recyclerIdMatch[1]));
    if (!recycler) return sendJson(response, 404, { error: "Recycler not found" });
    return sendJson(response, 200, { ...recycler, verified: Boolean(recycler.verified), pickup: Boolean(recycler.pickup) });
  }

  if (pathname === "/analytics" && request.method === "GET") return sendJson(response, 200, getAnalytics());
  ...
  if (pathname === "/lots" && request.method === "GET") {
    ...
    return sendJson(response, 200, lots.map((lot) => ({ ...lot, synced: Boolean(lot.synced) })));
  }
  ...
  if (pathname === "/lots" && request.method === "POST") {
    ...
    if (!material || !Number.isFinite(weight) || weight <= 0) {
      return sendJson(response, 400, { error: "A valid material and positive weight are required" });
    }
    ...
    database.prepare("INSERT INTO lots (lotId, materialId, material, weight, estimatedVal, collectorName, status, createdAt, synced) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(
      lot.lotId,
      lot.materialId,
      lot.material,
      lot.weight,
  ...
  if (lotStatusMatch && request.method === "PATCH") {
    ...
    if (!lot) return sendJson(response, 404, { error: "Lot not found" });
    if (!allowedStatuses.has(body.status)) return sendJson(response, 400, { error: "Unsupported lot status" });
    ...
    database.prepare("UPDATE lots SET status = ?, updatedAt = ?, paidAt = ? WHERE lotId = ?").run(body.status, updatedAt, paidAt, lot.lotId);
...
if (!process.env.VERCEL) {
  server.listen(port, () => console.log(`Kabadi Saathi API listening on http://localhost:${port}`));
}
